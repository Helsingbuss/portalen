...
import {
  ...
  sendUnpushedAgentNotifications,
  type AgentNotificationItem,
} from "../../services/agentNotificationsService";
...
export default function AgentNotificationsScreen() {
  const [items, setItems] = useState<AgentNotificationItem[]>([]);
  ...
  const loadNotifications = useCallback(async (refreshing = false) => {
    try {
      ...
      const result = await getMyAgentNotifications();
      setItems(result);
    } catch (error: any) {
  ...
  async function requestPermission() {
    try {
      ...
      if (permission.status === "granted") {
        ...
        Alert.alert("Notiser aktiverade", "Agentappen kan nu visa notiser.");
      } else {
  ...
  async function sendTestNotification() {
    try {
      ...
      await Notifications.scheduleNotificationAsync({
        content: {
          title: "Ny agentnotis",
          body: "Detta är en testnotis från Helsingbuss Agentapp.",
  ...
  async function sendRealUnreadNotifications() {
    try {
      ...
      const count = await sendUnpushedAgentNotifications();
  ...
  async function markRead(item: AgentNotificationItem) {
    try {
      ...
      >
        ...
        <View style={styles.heroCard}>
          ...
          <Text style={styles.heroText}>
            Se riktiga agentnotiser för offerter, bokningar och betalningar.
          </Text>
        </View>
