...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!id) {
    return res.status(400).json({ ok: false, error: "Hotell/boende-ID saknas." });
  }
  ...
  try {
    ...
    if (req.method === "PUT") {
      ...
      if (!updateData.name) {
        return res.status(400).json({
          ok: false,
          error: "Namn på hotell/boende saknas.",
        });
      }
    ...
    console.error("/api/admin/partners/hotell/[id] error", error);

    return res.status(500).json({
      ok: false,
      error: error?.message || "Kunde inte hantera hotell/boende.",
    });
  }
}
