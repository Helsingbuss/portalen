...
function buildUpdateData(body: any) {
  return {
    partner_type: cleanText(body.partner_type) || "operator",
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!id) {
    return res.status(400).json({ ok: false, error: "Operatörs-ID saknas." });
  }
  ...
  try {
    ...
    console.error("/api/admin/partners/operatorer/[id] error", error);

    return res.status(500).json({
      ok: false,
      error: error?.message || "Kunde inte hantera operatören.",
    });
  }
}
