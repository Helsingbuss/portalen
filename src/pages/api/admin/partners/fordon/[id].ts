...
function buildUpdateData(body: any) {
  const name = cleanText(body.name) || cleanText(body.vehicle_name);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === "PUT") {
      ...
      if (!updateData.vehicle_name && !updateData.name) {
        return res.status(400).json({
          ok: false,
    ...
    console.error("/api/admin/partners/fordon/[id] error", error);
