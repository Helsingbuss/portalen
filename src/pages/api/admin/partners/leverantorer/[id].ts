...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!id) {
    return res.status(400).json({ ok: false, error: "Leverantör-ID saknas." });
  }
  ...
  try {
    ...
    console.error("/api/admin/partners/leverantorer/[id] error", error);

    return res.status(500).json({
      ok: false,
      error: error?.message || "Kunde inte hantera leverantör.",
    });
  }
}
