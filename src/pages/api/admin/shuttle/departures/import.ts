...
async function getMaps(rows: ImportRow[]) {
  ...
  const { data: lineStops, error: lineStopsError } = await db
    ...
    .select("id,line_id,stop_id,stop_order");
  ...
  for (const lineStop of lineStops || []) {
    lineStopMap.set(`${lineStop.line_id}:${lineStop.stop_id}`, lineStop);
  }
...
async function buildPreview(rows: ImportRow[]) {
  const { lineMap, stopMap } = await getMaps(rows);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { stopMap, lineStopMap } = await getMaps(rows);

    const imported: any[] = [];
    const updated: any[] = [];
    const invalidRows = preview.rows.filter((row: any) => row.errors.length > 0);
    ...
    for (const row of validRows as any[]) {
      ...
      let departureId = existing?.id || null;
      ...
      const departureStops = row.stops.map((stop: any, index: number) => {
        ...
        const foundLineStop = foundStop ? lineStopMap.get(`${row.lineId}:${foundStop.id}`) : null;
    ...
    return res.status(200).json({
      ...
      updated,
      invalidRows,
      message: `${imported.length} nya avgångar skapades och ${updated.length} befintliga avgångar uppdaterades.`,
    });
  } catch (error: any) {
