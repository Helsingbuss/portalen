...
function weekKey(value: any) {
  ...
  if (!dateText) return "Okänd vecka";
...
function buildWeeks(rows: any[]) {
  const map = new Map<string, any>();

  for (const row of rows) {
    const key = row.week || "Okänd vecka";
    const existing =
      map.get(key) ||
      {
        week: key,
...
function buildSummary(weeks: any[], rows: any[]) {
  const revenue = weeks.reduce((sum, row) => sum + n(row.revenue), 0);
  const costs = weeks.reduce((sum, row) => sum + n(row.costs), 0);
  const result = revenue - costs;
  const paid = weeks.reduce((sum, row) => sum + n(row.paid_amount), 0);
  const unpaid = weeks.reduce((sum, row) => sum + n(row.unpaid_amount), 0);
  const bestWeek = [...weeks].sort((a, b) => n(b.result) - n(a.result))[0] || null;
  const worstWeek = [...weeks].sort((a, b) => n(a.result) - n(b.result))[0] || null;
  ...
  return {
    ...
    averageRevenuePerWeek:
      weeks.length > 0 ? Number((revenue / weeks.length).toFixed(2)) : 0,
    bestWeek,
    worstWeek,
  };
}

function toCsv(weeks: any[], rows: any[], summary: any, range: any) {
  const csvRows = [
    ["Rapport per vecka"],
    ["Period", range.start || "Alla", range.end || "Alla"],
    [],
    ["Sammanställning"],
    ["Antal veckor", summary.weekCount],
    ...
    ["Snittintäkt per vecka", csvMoney(summary.averageRevenuePerWeek)],
    [],
    ["Veckor"],
    ["Vecka", "Rader", "Intäkter", "Kostnader", "Resultat", "Betalt", "Obetalt", "Sundra", "Shuttle", "Beställningstrafik"],
    ...weeks.map((week) => [
      week.week,
      week.rows,
      csvMoney(week.revenue),
      csvMoney(week.costs),
      csvMoney(week.result),
      csvMoney(week.paid_amount),
      csvMoney(week.unpaid_amount),
      week.sundra,
      week.shuttle,
      week.bestallningstrafik,
    ]),
    [],
    ["Underlag"],
    ["Datum", "Vecka", "Typ", "Källa", "Område", "Namn", "Beskrivning", "Nummer", "Intäkt", "Kostnad", "Resultat", "Betalt", "Obetalt", "Status", "Länk"],
    ...rows.map((row) => [
      row.date,
      row.week,
      row.type,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const summary = buildSummary(weeks, rows);

    if (format === "csv") {
      const csv = toCsv(weeks, rows, summary, range);
      const filename = "rapport-per-vecka.csv";
    ...
    console.error("/api/admin/rapporter-analys/per-vecka error", error);

    return res.status(500).json({
      ok: false,
      error: error?.message || "Kunde inte hämta rapport per vecka.",
    });
  }
}
