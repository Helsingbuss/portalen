...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (error) return res.status(200).json({ ok: true, ticketTypes: fallbackTicketTypes });

  return res.status(200).json({ ok: true, ticketTypes: data && data.length > 0 ? data : fallbackTicketTypes });
}
