...
export default async function handler(
  ...
  try {
    // Dynamisk import så det funkar oavsett ESM/CJS
    ...
    // stöd både 'file' och 'image' + om det är array
    let file: any = files?.file || files?.image;
    ...
    // skapa bucket om den inte finns (ignorera fel om den redan finns)
    try {
    ...
    const ext =
      path.extname(file.originalFilename || file.newFilename || "") || ".bin";
    ...
    console.error("/api/trips/upload-media error:", e);
