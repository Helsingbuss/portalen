...
export default function IntaktsrapportPage() {
  ...
  const csvUrl = "/api/admin/rapporter-analys/intaktsrapport?format=csv&" + queryString;
  ...
  async function loadData() {
    try {
      ...
      const res = await fetch("/api/admin/rapporter-analys/intaktsrapport?" + queryString);
      const json = await res.json().catch(() => ({}));

      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte hämta intäktsrapport.");
      }
      ...
      setError(err?.message || "Kunde inte hämta intäktsrapport.");
    } finally {
      setLoading(false);
    }
  }
          ...
          <div className="w-full space-y-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                  Se intäkter per verksamhetsområde, kund, produkt, månad och betalstatus.
                </p>
              </div>
            ...
            {warnings.length > 0 && (
              <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900 shadow-sm">
                <strong>Info:</strong> Några frivilliga datakällor kunde inte hämtas. Rapporten använder de tabeller som finns.
              </section>
            )}
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              <SummaryCard label="Intäkter exkl moms" value={fmtMoney(summary.net)} tone="green" />
              <SummaryCard label="Moms" value={fmtMoney(summary.vat)} />
              <SummaryCard label="Intäkter inkl moms" value={fmtMoney(summary.gross)} tone="green" />
              <SummaryCard label="Betalt" value={fmtMoney(summary.paid)} tone="green" />
              <SummaryCard label="Obetalt" value={fmtMoney(summary.unpaid)} tone={Number(summary.unpaid || 0) > 0 ? "amber" : "green"} />
            </section>

            <section className="grid gap-6 xl:grid-cols-2">
              <TopList title="Intäkter per verksamhetsområde" rows={charts.byArea || []} />
              <TopList title="Intäkter per månad" rows={charts.byMonth || []} />
              <TopList title="Största kunder" rows={charts.byCustomer || []} />
              <TopList title="Produkter / fakturarader" rows={charts.byProduct || []} />
            </section>

            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-100 p-5">
                <h2 className="text-lg font-bold text-[#194C66]">Intäktsrader</h2>
                <p className="mt-1 text-sm text-slate-500">
                  Kundfakturor och fristående intäktstransaktioner för vald period.
                </p>
              </div>

              {loading ? (
                <div className="p-8 text-center text-sm text-slate-500">
                  Laddar intäktsrapport...
                </div>
              ) : rows.length === 0 ? (
                <div className="p-8 text-center text-sm text-slate-500">
                  Inga intäkter hittades för vald period.
                </div>
              ) : (
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {rows.map((row, index) => (
                        <tr key={row.id || index} className="align-top transition hover:bg-slate-50">
                          ...
                          <Td className="font-bold text-[#194C66]">{row.customer || "—"}</Td>
                          ...
                          <Td className="text-right font-black text-emerald-700">{fmtMoney(row.gross_amount)}</Td>
