...
export default function AgentrapportPage() {
  ...
  }, [period, agent, start, end]);

  const csvUrl = "/api/admin/rapporter-analys/agentrapport?format=csv&" + queryString;
  ...
  async function loadData() {
    try {
      ...
      const res = await fetch("/api/admin/rapporter-analys/agentrapport?" + queryString);
      const json = await res.json().catch(() => ({}));

      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte hämta agentrapport.");
      }
      ...
      setError(err?.message || "Kunde inte hämta agentrapport.");
    } finally {
      setLoading(false);
    }
  }
          ...
          <div className="w-full space-y-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                  Följ försäljning, intäkter och prestation per bokningsagent/säljare.
                </p>
              </div>
            ...
            {warnings.length > 0 && (
              <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900 shadow-sm">
                <strong>Info:</strong> Några frivilliga tabeller kunde inte hämtas. Rapporten använder de tabeller som finns.
              </section>
            )}
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-5 grid gap-4 xl:grid-cols-[190px_240px_180px_180px_auto] xl:items-end">
                ...
                <SelectField
                  ...
                  options={[
                    ["all", "Alla agenter"],
                    ...agents.map((item) => [String(item.agent_id), item.agent_name] as [string, string]),
                  ]}
                />
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              <SummaryCard label="Antal agenter" value={summary.agentCount || 0} />
              <SummaryCard label="Antal affärer/rader" value={summary.rowCount || 0} />
              <SummaryCard label="Intäkt exkl moms" value={fmtMoney(summary.totalRevenueNet)} tone="green" />
              <SummaryCard label="Betalt" value={fmtMoney(summary.totalPaid)} tone="green" />
              <SummaryCard label="Obetalt" value={fmtMoney(summary.totalUnpaid)} tone={Number(summary.totalUnpaid || 0) > 0 ? "amber" : "green"} />
            </section>

            <section className="grid gap-6 xl:grid-cols-[2fr_1fr]">
              <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="border-b border-slate-100 p-5">
                  <h2 className="text-lg font-bold text-[#194C66]">Agenter</h2>
                    ...
                    <table className="min-w-[1050px] w-full border-collapse text-left text-sm">
                      <thead>
                        <tr className="border-b border-slate-200">
                          ...
                          <Th className="text-right">Bekräftat %</Th>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {agents.map((item) => (
                          <tr key={item.agent_id} className="align-top transition hover:bg-slate-50">
                            <Td className="font-bold text-[#194C66]">{item.agent_name}</Td>
                            ...
                            <Td className="text-right">{item.conversion_hint || 0} %</Td>
                            <Td className="text-right">{item.area_sundra}</Td>
                            <Td className="text-right">{item.area_shuttle}</Td>
                            <Td className="text-right">{item.area_bestallningstrafik}</Td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <TopList title="Intäkt per område" rows={areaChart || []} />
            </section>
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-100 p-5">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Rader från kundfakturor, offerter och bokningar där agent/säljare kan identifieras.
                </p>
              </div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {rows.map((row, index) => (
                        <tr key={row.id || index} className="align-top transition hover:bg-slate-50">
                          <Td>{row.date || "—"}</Td>
                          <Td>{row.source || "—"}</Td>
                          <Td className="font-bold text-[#194C66]">{row.agent_name || "Ej angiven"}</Td>
                          <Td>
                          ...
                          <Td>{row.number || "—"}</Td>
                          <Td className="text-right font-black text-emerald-700">{fmtMoney(row.revenue_net)}</Td>
