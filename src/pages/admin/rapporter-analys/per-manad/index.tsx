...
export default function RapportPerManadPage() {
  ...
  const [months, setWeeks] = useState<any[]>([]);
  ...
  const csvUrl = "/api/admin/rapporter-analys/per-manad?format=csv&" + queryString;

  const maxValue = useMemo(() => {
    return Math.max(
      1,
      ...months.map((month) => Math.max(Number(month.revenue || 0), Number(month.costs || 0)))
    );
  }, [months]);
  ...
  async function loadData() {
    try {
      ...
      const res = await fetch("/api/admin/rapporter-analys/per-manad?" + queryString);
      const json = await res.json().catch(() => ({}));

      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte hämta rapport per månad.");
      }

      setSummary(json.summary || {});
      setWeeks(json.months || []);
      setRows(json.rows || []);
      setRange(json.range || {});
      setWarnings(json.warnings || []);
    } catch (err: any) {
      setError(err?.message || "Kunde inte hämta rapport per månad.");
    } finally {
      setLoading(false);
    }
  }
        ...
        <main className="px-6 pb-8 pt-10">
          <div className="w-full space-y-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                  Följ intäkter, kostnader, resultat och obetalda poster månad för månad.
                </p>
              </div>
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-7">
              <SummaryCard label="Månader" value={summary.monthCount || 0} />
              ...
              <SummaryCard label="Snitt/månad" value={fmtMoney(summary.averageRevenuePerMonth)} />
              <SummaryCard label="Obetalt" value={fmtMoney(summary.unpaid)} tone={Number(summary.unpaid || 0) > 0 ? "amber" : "green"} />
            </section>

            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-lg font-bold text-[#194C66]">Utveckling per månad</h2>

              {loading ? (
                <div className="mt-6 text-sm text-slate-500">Laddar...</div>
              ) : months.length === 0 ? (
                <div className="mt-6 text-sm text-slate-500">Ingen månadsdata hittades.</div>
              ) : (
                <div className="mt-6 space-y-4">
                  {months.map((month) => (
                    <div key={month.month}>
                      <div className="mb-1 flex items-center justify-between text-sm">
                        <span className="font-bold text-[#194C66]">{month.month}</span>
                        <span className={Number(month.result || 0) >= 0 ? "font-black text-emerald-700" : "font-black text-red-700"}>
                          {fmtMoney(month.result)}
                        </span>
                      </div>

                      <div className="grid gap-1">
                        <Bar label="Intäkter" value={Number(month.revenue || 0)} max={maxValue} tone="green" />
                        <Bar label="Kostnader" value={Number(month.costs || 0)} max={maxValue} tone="red" />
                      </div>

                      <div className="mt-1 text-xs text-slate-500">
                        {month.rows} rader · Sundra {month.sundra} · Shuttle {month.shuttle} · Beställning {month.bestallningstrafik}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-100 p-5">
                <h2 className="text-lg font-bold text-[#194C66]">Månadstabell</h2>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-[1100px] w-full border-collapse text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {months.map((month) => (
                        <tr key={month.month} className="transition hover:bg-slate-50">
                          <Td className="font-bold text-[#194C66]">{month.month}</Td>
                          <Td className="text-right">{month.rows}</Td>
                          <Td className="text-right font-bold text-emerald-700">{fmtMoney(month.revenue)}</Td>
                          <Td className="text-right font-bold text-red-700">{fmtMoney(month.costs)}</Td>
                          <Td className={Number(month.result || 0) >= 0 ? "text-right font-black text-emerald-700" : "text-right font-black text-red-700"}>
                            {fmtMoney(month.result)}
                          </Td>
                          <Td className="text-right">{fmtMoney(month.paid_amount)}</Td>
                          <Td className="text-right">{fmtMoney(month.unpaid_amount)}</Td>
                          <Td className="text-right">{month.sundra}</Td>
                          <Td className="text-right">{month.shuttle}</Td>
                          <Td className="text-right">{month.bestallningstrafik}</Td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
