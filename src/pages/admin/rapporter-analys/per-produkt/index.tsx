...
export default function RapportPerProduktPage() {
  ...
  }, [period, area, product, start, end]);

  const csvUrl = "/api/admin/rapporter-analys/per-produkt?format=csv&" + queryString;
  ...
  async function loadData() {
    try {
      ...
      const res = await fetch("/api/admin/rapporter-analys/per-produkt?" + queryString);
      const json = await res.json().catch(() => ({}));

      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte hämta rapport per produkt.");
      }
      ...
      setError(err?.message || "Kunde inte hämta rapport per produkt.");
    } finally {
      setLoading(false);
    }
  }
          ...
          <div className="w-full space-y-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                  Följ produkter, resor, biljettintäkter, kostnader, resultat och verksamhetsområde.
                </p>
              </div>
            ...
            {warnings.length > 0 && (
              <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900 shadow-sm">
                <strong>Info:</strong> Några produkt-/fakturaradstabeller kunde inte hämtas. Rapporten använder de tabeller som finns.
              </section>
            )}
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-5 grid gap-4 xl:grid-cols-[180px_220px_280px_170px_170px_auto] xl:items-end">
                <SelectField
                  label="Period"
                  value={period}
                  onChange={setPeriod}
                ...
                <SelectField
                  ...
                  options={[
                    ["all", "Alla produkter"],
                    ...products.map((item) => [String(item.product_id), item.product] as [string, string]),
                  ]}
                />
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-7">
              <SummaryCard label="Produkter" value={summary.productCount || 0} />
              <SummaryCard label="Rader" value={summary.rowCount || 0} />
              <SummaryCard label="Antal/st" value={summary.quantityTotal || 0} />
            ...
            <section className="grid gap-6 xl:grid-cols-[2fr_1fr]">
              <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="border-b border-slate-100 p-5">
                  <h2 className="text-lg font-bold text-[#194C66]">Produkter</h2>
                  <p className="mt-1 text-sm text-slate-500">
                    Sorterat efter intäkt.
                  </p>
                </div>

                {loading ? (
                  <div className="p-8 text-center text-sm text-slate-500">
                    Laddar produktrapport...
                  </div>
                ) : products.length === 0 ? (
                  <div className="p-8 text-center text-sm text-slate-500">
                    Ingen produktdata hittades.
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="min-w-[1200px] w-full border-collapse text-left text-sm">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {products.map((item) => (
                          <tr key={item.product_id} className="align-top transition hover:bg-slate-50">
                            <Td className="font-bold text-[#194C66]">{item.product}</Td>
                            <Td>{item.code || "—"}</Td>
                            <Td className="text-right">{item.rows}</Td>
                            <Td className="text-right">{item.quantity}</Td>
                            <Td className="text-right font-bold text-emerald-700">{fmtMoney(item.revenue)}</Td>
                            <Td className="text-right font-bold text-red-700">{fmtMoney(item.costs)}</Td>
                            <Td className={Number(item.result || 0) >= 0 ? "text-right font-black text-emerald-700" : "text-right font-black text-red-700"}>
                              {fmtMoney(item.result)}
                            </Td>
                            <Td className="text-right">{fmtMoney(item.average_price)}</Td>
                            <Td className="text-right">{item.margin_percent || 0} %</Td>
              ...
              <TopList title="Produkter per område" rows={areaChart || []} />
            </section>
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-100 p-5">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Kundfakturarader och leverantörsrader som ligger till grund för produktrapporten.
                </p>
              </div>
            ...
            <div key={row.label + index} className="border-b border-slate-100 pb-3 last:border-0 last:pb-0">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <div className="text-xs text-slate-500">{row.rows} rader · {row.quantity} antal</div>
                </div>

                <div className="text-right">
                  <div className="font-black text-[#00645d]">{fmtMoney(row.revenue)}</div>
                  <div className={Number(row.result || 0) >= 0 ? "text-xs font-bold text-emerald-700" : "text-xs font-bold text-red-700"}>
                    {fmtMoney(row.result)}
                  </div>
                </div>
              </div>
