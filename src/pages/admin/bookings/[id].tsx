...
function clsStatusPill(s?: string | null) {
  ...
  if (v === "bekraftad" || v === "bekräftad" || v === "confirmed")
    return "bg-emerald-100 text-emerald-800";
  if (v === "planerad" || v === "bokad" || v === "created")
...
function prioMeta(date?: string | null, time?: string | null) {
  ...
  if (!target)
    return {
      ...
      title: "Saknar/ogiltigt datum",
    };

  const diffH = (target.getTime() - Date.now()) / 36e5;
  if (diffH <= RED_HOURS)
    return { label: "Röd", cls: "bg-red-100 text-red-800", title: "≤ 48h kvar" };
  if (diffH <= ORANGE_HOURS)
    return {
      label: "Orange",
...
function bookingToForm(b: Booking): BookingForm {
  return {
    ...
    return_on_site_minutes:
      b.return_on_site_minutes == null ? "" : String(b.return_on_site_minutes),
...
function formToPatch(f: BookingForm) {
  ...
  return {
    ...
    return_on_site_minutes:
      f.return_on_site_minutes === "" ? null : Number(f.return_on_site_minutes),
...
export default function BookingDetailPage() {
  ...
  const canCreateOrder =
    b && !["inställd", "makulerad"].includes((b.status || "").toLowerCase());
  ...
  async function deleteBooking() {
    if (!b) return;
    const ok = confirm(
      "Är du säker på att du vill ta bort bokningen? Detta går inte att ångra."
    );
    if (!ok) return;
        ...
        <main className="p-6 pt-24 space-y-6">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap py-1">
              ...
              {b && (
                <a
                  href={`/admin/avvikelser/new?booking_number=${encodeURIComponent(
                    b.booking_number || ""
                  )}&booking_id=${encodeURIComponent(b.id || "")}`}
                  className="px-4 py-2 rounded-[25px] bg-orange-600 text-white text-sm"
                  title="Skapa avvikelse för denna bokning"
                >
                  Skapa avvikelse
                </a>
              )}
                  ...
                  <div className="bg-[#f8fafc] rounded-lg p-4">
                    ...
                    <div className="text-[#194C66] space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs ${clsStatusPill(
                            b.status
                          )}`}
                        >
                          {b.status || "—"}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          title={prio.title}
                          className={`px-2 py-0.5 rounded-full text-xs ${prio.cls}`}
                        >
                          {prio.label}
                        </span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {b.customer_phone ? (
                          <>
                            <a
                              className="underline"
                              href={`tel:${sanitizeTel(b.customer_phone)}`}
                            >
                              {b.customer_phone}
                            </a>
                            <button
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[#f8fafc] rounded-lg p-4">
                    ...
                    <div className="text-[#194C66] space-y-1">
                      ...
                      <div>
                        <span className="font-semibold">Slut (planerat):</span>{" "}
                        {fmtTime(b.end_time)}
                      </div>
                      <div>
                        <span className="font-semibold">På plats:</span>{" "}
                        {hasOnSite(b.on_site_minutes)
                          ? `${Math.max(0, b.on_site_minutes || 0)} min före`
                          : "—"}
                      </div>
                      <div>
                  ...
                  <div className="bg-[#f8fafc] rounded-lg p-4">
                    ...
                    <div className="text-[#194C66] space-y-1">
                      <div>
                        <span className="font-semibold">Datum:</span>{" "}
                        {fmtDateSv(b.return_date)}
                      </div>
                      <div>
                        <span className="font-semibold">Start:</span>{" "}
                        {fmtTime(b.return_time)}
                      </div>
                      <div>
                        <span className="font-semibold">Slut (planerat):</span>{" "}
                        {fmtTime(b.return_end_time)}
                      </div>
                      <div>
                        <span className="font-semibold">På plats:</span>{" "}
                        {hasOnSite(b.return_on_site_minutes)
                          ? `${Math.max(0, b.return_on_site_minutes || 0)} min före`
                          : "—"}
                      </div>
                      <div>
                        <span className="font-semibold">Från:</span>{" "}
