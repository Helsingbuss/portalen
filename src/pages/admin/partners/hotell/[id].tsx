...
export default function PartnerHotellDetailPage() {
  ...
  async function loadHotel() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/partners/hotell/" + encodeURIComponent(id));
      const json = await res.json().catch(() => ({}));

      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte hämta hotell/boende.");
      }

      const hotel = json.hotel || {};

      setCreatedAt(hotel.created_at || "");

      setForm({
        name: hotel.name || "",
        org_number: hotel.org_number || "",
        ...
        quality_level: hotel.quality_level || "normal",
        notes: hotel.notes || "",
      });
    } catch (err: any) {
  ...
  async function saveHotel(event: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte spara hotell/boende.");
      }

      setMessage("Hotell/boende sparades.");
      await loadHotel();
    } catch (err: any) {
      setError(err?.message || "Kunde inte spara hotell/boende.");
    } finally {
      setSaving(false);
    }
  }
          ...
          <div className="w-full space-y-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <h1 className="mt-2 text-3xl font-bold tracking-tight text-[#194C66]">
                  {loading ? "Hotell & boenden" : form.name || "Hotell/boende"}
                </h1>
            ...
            {loading ? (
              <section className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-slate-500 shadow-sm">
                Laddar hotell/boende...
              </section>
            ) : (
              <form id="hotel-edit-form" onSubmit={saveHotel} className="space-y-6">
