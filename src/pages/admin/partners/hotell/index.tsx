...
export default function PartnerHotellPage() {
  ...
  async function loadHotels() {
    try {
      ...
      const res = await fetch("/api/admin/partners/hotell?" + params.toString());
      const json: ApiResponse = await res.json().catch(() => ({ ok: false }));

      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte hämta hotell och boenden.");
      }
  ...
  async function createHotel(event: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/partners/hotell", {
        method: "POST",
        headers: {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte skapa hotell/boende.");
      }

      setMessage("Hotell/boende sparades.");
      setForm(emptyForm);
      setShowForm(false);
      await loadHotels();
    } catch (err: any) {
      setError(err?.message || "Kunde inte skapa hotell/boende.");
    } finally {
      setSaving(false);
    }
  }
  ...
  const filteredTotal = useMemo(() => hotels.length, [hotels]);
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm text-slate-600">
                  Här samlar du hotell, boenden och logipartners som kan kopplas till paketresor, evenemang, kryssningar och samarbeten.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                >
                  {showForm ? "Stäng formulär" : "Nytt hotell/boende"}
                </button>
              ...
              >
                <div className="flex flex-col gap-2 border-b border-slate-100 pb-4">
                  ...
                  <p className="text-sm text-slate-500">
                    Lägg in boende, kontaktperson, ort, status och anteckningar.
                  </p>
                </div>

                <div className="mt-5 grid gap-4 lg:grid-cols-3">
                  <Field label="Hotell / boende" value={form.name} onChange={(value) => updateField("name", value)} />
                ...
                <div className="mt-5 flex justify-end gap-3">
                  ...
                  >
                    {saving ? "Sparar..." : "Spara hotell/boende"}
                  </button>
                </div>
              </form>
            )}
              ...
              <div className="grid gap-4 lg:grid-cols-[1fr_220px_220px_140px]">
                <div>
                  ...
                  <input
                    ...
                    onKeyDown={(event) => {
                      if (event.key === "Enter") loadHotels();
                    }}
                    placeholder="Sök hotell, kontaktperson, e-post, ort eller anteckning..."
                    className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-[#194C66] focus:ring-2 focus:ring-[#194C66]/10"
                  />
                </div>
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Visar {filteredTotal} hotell/boenden
                  </p>
                </div>
              </div>
                ...
                <table className="min-w-[1120px] w-full border-collapse text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {loading ? (
                        ...
                        <td colSpan={7} className="px-5 py-10 text-center">
                          <div className="mx-auto max-w-lg">
                            <div className="text-base font-bold text-[#194C66]">
                              Inga hotell eller boenden hittades
                            </div>
                            <p className="mt-2 text-sm leading-6 text-slate-500">
                              Skapa första boendet med knappen Nytt hotell/boende.
                            </p>
                          </div>
                        </td>
                      </tr>
                      ...
                      hotels.map((hotel) => (
                        <tr key={hotel.id} onClick={() => router.push("/admin/partners/hotell/" + encodeURIComponent(hotel.id))} className="cursor-pointer align-top transition hover:bg-slate-50">
                          <Td>
                            <div className="font-bold text-[#194C66]">
                              {hotel.name || "Hotell/boende"}
                            </div>
                            {hotel.org_number && (
                          ...
                          <Td>
                            <span
                              className={
                                "inline-flex rounded-full px-3 py-1 text-xs font-semibold " +
                                qualityClass(hotel.quality_level)
                              }
                            >
                              {qualityLabel(hotel.quality_level)}
                            </span>
                          </Td>
                          ...
                          <Td>
                            {hotel.website ? (
                              <a
                                href={String(hotel.website).startsWith("http") ? hotel.website : "https://" + hotel.website}
