...
export default function PartnerLeverantörPage() {
  const router = useRouter();
  const [suppliers, setHotels] = useState<SupplierRow[]>([]);
  ...
  async function loadSuppliers() {
    try {
      ...
      const res = await fetch("/api/admin/partners/leverantorer?" + params.toString());
      const json: ApiResponse = await res.json().catch(() => ({ ok: false }));

      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte hämta leverantörer.");
      }

      setHotels(json.suppliers || []);
  ...
  async function createSupplier(event: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/partners/leverantorer", {
        method: "POST",
        headers: {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte skapa leverantör.");
      }

      setMessage("Leverantör sparades.");
      setForm(emptyForm);
      setShowForm(false);
      await loadSuppliers();
    } catch (err: any) {
      setError(err?.message || "Kunde inte skapa leverantör.");
    } finally {
      setSaving(false);
    }
  }
  ...
  const filteredTotal = useMemo(() => suppliers.length, [suppliers]);
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm text-slate-600">
                  Här samlar du leverantör, leverantörn och logipartners som kan kopplas till paketresor, evenemang, kryssningar och samarbeten.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                >
                  {showForm ? "Stäng formulär" : "Ny leverantör"}
                </button>
              ...
              >
                <div className="flex flex-col gap-2 border-b border-slate-100 pb-4">
                  ...
                  <p className="text-sm text-slate-500">
                    Lägg in leverantör, kontaktperson, ort, status och anteckningar.
                  </p>
                </div>

                <div className="mt-5 grid gap-4 lg:grid-cols-3">
                  <Field label="Leverantör" value={form.name} onChange={(value) => updateField("name", value)} />
              ...
              <div className="grid gap-4 lg:grid-cols-[1fr_220px_220px_140px]">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Sök leverantör, kontaktperson, e-post, ort eller anteckning..."
                    className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-[#194C66] focus:ring-2 focus:ring-[#194C66]/10"
                  />
                </div>
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Visar {filteredTotal} leverantörn
                  </p>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {loading ? (
                        ...
                        <td colSpan={7} className="px-5 py-10 text-center">
                          <div className="mx-auto max-w-lg">
                            ...
                            <p className="mt-2 text-sm leading-6 text-slate-500">
                              Skapa första leverantört med knappen Ny leverantör.
                            </p>
                          </div>
                        </td>
                      </tr>
                      ...
                      suppliers.map((supplier) => (
                        <tr key={supplier.id} onClick={() => router.push("/admin/partners/leverantorer/" + encodeURIComponent(supplier.id))} className="cursor-pointer align-top transition hover:bg-slate-50">
                          <Td>
                          ...
                          <Td>
                            <span
                              className={
                                "inline-flex rounded-full px-3 py-1 text-xs font-semibold " +
                                qualityClass(supplier.quality_level)
                              }
                            >
                              {qualityLabel(supplier.quality_level)}
                            </span>
                          </Td>
                          ...
                          <Td>
                            {supplier.website ? (
                              <a
                                href={String(supplier.website).startsWith("http") ? supplier.website : "https://" + supplier.website}
