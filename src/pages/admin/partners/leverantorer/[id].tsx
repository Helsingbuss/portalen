...
export default function PartnerLeverantörDetailPage() {
  ...
  async function loadSupplier() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/partners/leverantorer/" + encodeURIComponent(id));
      const json = await res.json().catch(() => ({}));

      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte hämta leverantör.");
      }
      ...
      setForm({
        ...
        quality_level: supplier.quality_level || "normal",
  ...
  async function saveSupplier(event: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte spara leverantör.");
      }

      setMessage("Leverantör sparades.");
      await loadSupplier();
    } catch (err: any) {
      setError(err?.message || "Kunde inte spara leverantör.");
    } finally {
      setSaving(false);
    }
  }
        ...
        <main className="px-6 pb-8 pt-10">
          <div className="w-full space-y-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <h1 className="mt-2 text-3xl font-bold tracking-tight text-[#194C66]">
                  {loading ? "Leverantörer" : form.name || "Leverantör"}
                </h1>
              ...
              <div className="flex flex-wrap gap-3">
                <Link
                  href="/admin/partners/leverantorer"
            ...
            {loading ? (
              ...
              <form id="supplier-edit-form" onSubmit={saveSupplier} className="space-y-6">
