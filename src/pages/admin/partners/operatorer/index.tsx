...
export default function PartnerOperatorerPage() {
  ...
  async function loadOperators() {
    try {
      ...
      const res = await fetch("/api/admin/partners/operatorer?" + params.toString());
      const json: ApiResponse = await res.json().catch(() => ({ ok: false }));

      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte hämta operatörer.");
      }
  ...
  async function createOperator(event: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/partners/operatorer", {
        method: "POST",
        headers: {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte skapa operatör.");
      }
      ...
      setError(err?.message || "Kunde inte skapa operatör.");
    } finally {
      setSaving(false);
    }
  }
  ...
  const filteredTotal = useMemo(() => operators.length, [operators]);
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm text-slate-600">
                  Här samlar du bussbolag och operatörer som kan kopplas till resor, avgångar, uppdrag och partnerflöden i appen.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                >
                  {showForm ? "Stäng formulär" : "Ny operatör"}
                </button>
            ...
            {needsSetup && (
              <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-800 shadow-sm">
                <h2 className="text-base font-bold">
                  Appens partner-tabell saknas ännu
                </h2>
                <p className="mt-2 text-sm leading-6">
                  Tabellen <strong>app_partners</strong> saknas i databasen. Eftersom appen redan är byggd mot den tabellen bör vi skapa/återställa den innan portalen används.
                </p>
              </section>
            )}
              ...
              >
                <div className="flex flex-col gap-2 border-b border-slate-100 pb-4">
                  ...
                  <p className="text-sm text-slate-500">
                    Lägg in bussbolag, kontaktperson, status och kvalitetsnivå.
                  </p>
                </div>
              ...
              <div className="grid gap-4 lg:grid-cols-[1fr_220px_220px_140px]">
                <div>
                  ...
                  <input
                    ...
                    onKeyDown={(event) => {
                      if (event.key === "Enter") loadOperators();
                    }}
                    placeholder="Sök operatör, kontaktperson, e-post, ort eller org.nr..."
                    className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-[#194C66] focus:ring-2 focus:ring-[#194C66]/10"
                  />
                </div>
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Visar {filteredTotal} operatörer
                  </p>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {loading ? (
                        ...
                        <td colSpan={7} className="px-5 py-10 text-center">
                          <div className="mx-auto max-w-lg">
                            ...
                            <p className="mt-2 text-sm leading-6 text-slate-500">
                              Skapa första operatören med knappen Ny operatör.
                            </p>
                          </div>
                        </td>
                      </tr>
                      ...
                      operators.map((operator) => (
                        <tr key={operator.id} onClick={() => router.push("/admin/partners/operatorer/" + encodeURIComponent(operator.id))} className="cursor-pointer align-top transition hover:bg-slate-50">
                          <Td>
                          ...
                          <Td>
                            <span
                              className={
                                "inline-flex rounded-full px-3 py-1 text-xs font-semibold " +
                                qualityClass(operator.quality_level)
                              }
                            >
                              {qualityLabel(operator.quality_level)}
                            </span>
                          </Td>
                          ...
                          <Td>
                            {operator.website ? (
                              <a
                                href={String(operator.website).startsWith("http") ? operator.website : "https://" + operator.website}
                                target="_blank"
                                rel="noreferrer"
