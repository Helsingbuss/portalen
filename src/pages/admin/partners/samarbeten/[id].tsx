...
export default function PartnerSamarbetenDetailPage() {
  ...
  async function loadPartnership() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/partners/samarbeten/" + encodeURIComponent(id));
      const json = await res.json().catch(() => ({}));

      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte hämta samarbete/partner.");
      }

      const partnership = json.partnership || {};

      setCreatedAt(partnership.created_at || "");

      setForm({
        name: partnership.name || "",
        org_number: partnership.org_number || "",
        ...
        quality_level: partnership.quality_level || "normal",
  ...
  async function savePartnership(event: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Kunde inte spara samarbete/partner.");
      }

      setMessage("Samarbete / partner sparades.");
      await loadPartnership();
    } catch (err: any) {
      setError(err?.message || "Kunde inte spara samarbete/partner.");
    } finally {
      setSaving(false);
    }
  }
          ...
          <div className="w-full space-y-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <h1 className="mt-2 text-3xl font-bold tracking-tight text-[#194C66]">
                  {loading ? "Samarbeten / partnerskap" : form.name || "Samarbete / partner"}
                </h1>
              ...
              <div className="flex flex-wrap gap-3">
                <Link
                  href="/admin/partners/samarbeten"
                ...
                {form.website && (
                  <a
                    href={String(form.website).startsWith("http") ? form.website : "https://" + form.website}
                    target="_blank"
                    rel="noreferrer"
            ...
            {loading ? (
              <section className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-slate-500 shadow-sm">
                Laddar samarbete/partner...
              </section>
            ) : (
              <form id="partnership-edit-form" onSubmit={savePartnership} className="space-y-6">
                ...
                <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  ...
                  <textarea
                    ...
                    rows={8}
                    placeholder="Ex. antal rum, prisnivå, frukost, bussparkering, kontaktväg, paketresor..."
