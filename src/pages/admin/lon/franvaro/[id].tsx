...
export default function LonFranvaroDetailPage() {
  ...
  async function loadAbsence() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/lon/franvaro/" + encodeURIComponent(id));
      ...
      const absence = json.absence || {};

      setEmployees(json.employees || []);
      setCreatedAt(absence.created_at || "");
      setUpdatedAt(absence.updated_at || "");

      setForm({
        employee_id: absence.employee_id || "",
        absence_type: absence.absence_type || "sick",
        start_date: absence.start_date ? String(absence.start_date).slice(0, 10) : "",
        end_date: absence.end_date ? String(absence.end_date).slice(0, 10) : "",
        start_time: absence.start_time || "",
        end_time: absence.end_time || "",
        hours: absence.hours !== null && absence.hours !== undefined ? String(absence.hours) : "0",
        paid_percent: absence.paid_percent !== null && absence.paid_percent !== undefined ? String(absence.paid_percent) : "0",
        affects_payroll: absence.affects_payroll !== false,
    ...
    <>
      ...
      <div className="min-h-screen bg-[#f5f4f0] lg:pl-64">
        ...
        <main className="px-6 pb-8 pt-10">
          <form onSubmit={saveAbsence} className="w-full space-y-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <h1 className="mt-2 text-3xl font-bold tracking-tight text-[#194C66]">
                  {loading ? "Frånvaro" : employeeName(employee)}
                </h1>

                <p className="mt-2 max-w-3xl text-sm text-slate-600">
                  {typeLabel(form.absence_type)} · {statusLabel(form.status)}
                </p>
              </div>

              <div className="flex flex-wrap gap-3">
                <Link
                  href="/admin/lon/franvaro"
